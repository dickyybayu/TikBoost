...
class TopProductsService {
    ...
    try {
      ...
      if (username.isNotEmpty) {
        ...
        if (backendProducts != null) {
          ...
          _userTopProducts = backendProducts.map((json) => Product(
            id: json['id'] ?? '',
            name: json['name'] ?? '',
        ...
        print('⚠️ No username available, trying local storage...');
      }
      ...
      if (productsJson != null) {
        ...
        _userTopProducts = productsList.map((json) => Product(
          id: json['id'] ?? '',
          name: json['name'] ?? '',
          price: (json['price'] ?? 0.0).toDouble(),
          salesCount: json['salesCount'] ?? 0,
  ...
  static Future<void> saveProducts() async {
    try {
      ...
      final productsJson = _userTopProducts.map((product) => {
        'id': product.id,
        'name': product.name,
        'price': product.price,
      ...
      if (username.isNotEmpty) {
        final apiService = ApiService();
        final success = await apiService.saveUserProducts(productsJson, username);
